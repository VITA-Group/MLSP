...
class DGCNN_DefRec(nn.Module):
    def __init__(self, args, in_size=3, num_classes=8):
        ...
        self.Norm_pred = Normal_prediction(args, self.num_f_prev + 1024)
        self.Density_cls = Density_prediction(args, self.num_f_prev + 1024)

    def forward(self, x, make_seg=True, activate_DefRec=True, activate_normal=False, activate_density=False, activate_density_normal_ondef=False):
        ...
        # print(x123.shape,x5.shape) [16, 192, 2048]) torch.Size([16, 1024, 1]
        x = torch.cat((x123, x5.repeat(1, 1, num_points)), dim=1)
        # print(x.shape) [16, 1216, 2048]
        if make_seg:
        ...
        if activate_normal:
            logits["Normal"] = self.Norm_pred(x)
        if activate_density:
            logits['density'], logits['density_mse'] = self.Density_cls(x)
        if activate_density_normal_ondef:
            logits["DefRec"] = self.DefRec(x)
            logits['density'], logits['density_mse'] = self.Density_cls(x)
            logits["Normal"] = self.Norm_pred(x)
...
class Density_prediction(nn.Module):
    def __init__(self, args, input_size):
        ...
        self.mlp1 = fc_layer_v2(512, 256, bias=bias, activation=activate, bn=True)
        self.dp1 = nn.Dropout(p=args.dropout)
        self.mlp2 = fc_layer_v2(256, 256, bias=True, activation=activate, bn=True)
        self.dp2 = nn.Dropout(p=args.dropout)
